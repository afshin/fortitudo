...
JupyterLab extension. `llvm-explorer --no-browser` serves this packaged site on
...
The deployed URLs are `https://llvm-explorer.dev/` and
`https://llvm-explorer.dev/lite/lab/index.html`. Set the repository's Pages
custom domain to `llvm-explorer.dev` and enable HTTPS after DNS validation.
Actions deployments use that setting; no `CNAME` file is needed in the
repository. Generated site and kernel assets are uploaded as an Actions
artifact; they do not belong in Git.
...
```sh
pixi run --as-is jlpm pack --out dist/llvm-explorer.tgz
...
work/wheel-env/bin/python -m pip install --no-index --no-deps \
  dist/llvm_explorer-*.whl
work/wheel-env/bin/python -I -m unittest discover -s tests -v
LLVM_EXPLORER_LOCAL_COMMAND=work/wheel-env/bin/llvm-explorer \
...
If installing a release in Pixi fails on
`labextensions/llvm-explorer/package.json`, run
`unlink .pixi/envs/default/share/jupyter/labextensions/llvm-explorer` from the
repository root to remove the broken development link, then retry with
`pixi run --as-is python -m pip install --force-reinstall llvm-explorer`.
