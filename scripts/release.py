...
dist = root / 'dist'
npm = dist / 'llvm-explorer.tgz'
sdist = dist / f'llvm_explorer-{version}.tar.gz'
wheel = dist / f'llvm_explorer-{version}-py3-none-any.whl'
...
def check_metadata(data, label):
    ...
    if metadata['Name'] != 'llvm-explorer' or metadata['Version'] != version:
...
with tarfile.open(sdist) as archive:
    member = archive.extractfile(f'llvm_explorer-{version}/PKG-INFO')
    if member is None:
...
with zipfile.ZipFile(wheel) as archive:
    check_metadata(
        archive.read(f'llvm_explorer-{version}.dist-info/METADATA'), wheel.name
    )
