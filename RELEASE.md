# Releasing LLVM Explorer

Publishing a stable GitHub release runs `publish-release.yml`, which builds,
checks, and publishes LLVM Explorer to npm and PyPI using trusted publishing.
...
5. Confirm that the workflow succeeds and the expected version appears on
   [npm](https://www.npmjs.com/package/llvm-explorer) and
   [PyPI](https://pypi.org/project/llvm-explorer/). Install that exact Python
   version in a fresh Python environment. Run `llvm-explorer`, compile a
   function, and run both example notebooks through **Try in Jupyter**. Also
   check the extension in a JupyterLab environment.
...
| Artifact               | Contents                                     |
| ---------------------- | -------------------------------------------- |
| `npm-distribution`     | The npm archive, `llvm-explorer.tgz`         |
...
Trusted publishing uses `afshin/llvm-explorer`, `publish-release.yml`, and the
