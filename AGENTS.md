...
```bash
python -m py_compile fortitudo/__init__.py  # Check single file for syntax errors
```
...
```typescript
const PLUGIN_ID = 'fortitudo:plugin';
```
...
export namespace CommandIDs {
  export const openPanel = 'fortitudo:open-panel';
  export const refreshData = 'fortitudo:refresh-data';
}
...
- **Output**: Creates files in `lib/` and `fortitudo/labextension/`
...
- Ask user to search for the extension ID (`fortitudo`) to see if it loaded
...
ls -la fortitudo/labextension/  # Should contain bundled extension
```
...
- **Plugin ID** (in `src/index.ts`): `'fortitudo:plugin'`
- **Command IDs** (in `src/commands.ts` or `src/index.ts`): `'fortitudo:command-name'`
...
- **CSS classes** (in `style/index.css`): `.jp-fortitudo-ClassName`
